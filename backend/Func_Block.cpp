...
bool Func_Block::isRunning {true};
void Func_Block::setRunning(bool v)
{
    Func_Block::isRunning = v;
}
...
void Func_Block::runFunc(FuncBody_info_t &toRun, int which)
{
    ...
    // also need to remember the steppingCounter position
    std::cout << name << " calling " << toRun.bodyFuncs[which].func->name << std::endl; // for debugging
    
    if (!Func_Block::isRunning)
    {
        system("PAUSE");
        // here it is possible to use QEventLoop to do some thing like system pause
    }
    ...
    // step 2, run/step in the proper next properly
...
void Func_Block::funcBodyHandler(FuncBody_info_t &body)
{
    switch (body.statType)
    {
    case bodyType_e::FUNCCALL:
        runFunc(body);
        break;
    case bodyType_e::WHILE:
        while((**(body.condition)).val._bool) { runFunc(body); }
        break;
    case bodyType_e::IF:
        if ((**(body.condition)).val._bool) { runFunc(body, 0); }
        else { runFunc(body, 1); }
        break;
